...
class FileSystem {
  public:
    ...
    for (int i = 0; i < 20; i++) StatusTable[i] = 0;  
    nowID = 1;
    }
...
//  The OpenAFile function is used for kernel open system call
    OpenFileId OpenAFile(char *name) {
        ...
        fileDescriptorTable[nowID] = new OpenFile(fileDescriptor);
        return nowID;
    }
    int WriteFile(char *buffer, int size, OpenFileId id){
        int returnsize = fileDescriptorTable[id]->Write(buffer, size);
        if(returnsize <=0) return -1;
        else return returnsize;
    }
    int ReadFile(char *buffer, int size, OpenFileId id){
        int returnsize = fileDescriptorTable[id]->Read(buffer, size);
        if(returnsize <0) return -1;
        else return returnsize;
    }
