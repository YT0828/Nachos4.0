...
Semaphore::P()
{
    ...
    DEBUG(dbgSelf, "T["<<currentThread->getID()<< "] wait for lock/Waitfor");
...
Semaphore::V()
{
    ...
    if (!queue->IsEmpty()) {  // make thread ready.
        tmp = queue->RemoveFront();
        DEBUG(dbgSelf, "In Semaphore::V(), T["<<tmp->getID()<< "] is push back to ready queue");
...
void Lock::Acquire()
{
    DEBUG(dbgSelf, "someone aquire lock");
    semaphore->P();
    lockHolder = kernel->currentThread;
    DEBUG(dbgSelf, "T["<<lockHolder->getID()<< "] hold lock and going to waitfor");
}
...
void Lock::Release()
{
    ...
    DEBUG(dbgSelf, "T["<<lockHolder->getID()<< "] hold lock and is going to release");
    lockHolder = NULL;
    semaphore->V();
}
