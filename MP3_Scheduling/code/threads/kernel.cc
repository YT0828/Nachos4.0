...
Kernel::Kernel(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-rs") == 0) {
            ...
            processPriority[execfileNum] = 0;
			cout << execfile[execfileNum] << "\n";
		} else if (strcmp(argv[i], "-ep") == 0) {
            execfile[++execfileNum] = argv [++i];
            processPriority[execfileNum] = atoi(argv[++i]);
            cout << execfile[execfileNum] << "\n";
...
Kernel::Initialize()
{
    ...
    currentThread ->approximate_burst_time = 0;
...
void Kernel::ExecAll()
{
	for (int i=1;i<=execfileNum;i++) {
		int a = Exec(execfile[i], processPriority[i]);
	}
...
int Kernel::Exec(char* name, int processPriority)
{
	t[threadNum] = new Thread(name, threadNum, processPriority);
