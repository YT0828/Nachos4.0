...
Thread::Thread(char* threadName, int threadID, int given_priority)
{
...
{
    //TODO
    DEBUG(dbgSelf, "preemtive: "<<kernel->scheduler->preemtive);
    if (priority >=50 && kernel->scheduler->preemtive == 0){
        return;
    }else if(priority < 50 && kernel->scheduler->L3->IsEmpty() &&kernel->scheduler->L2->IsEmpty()&& kernel->scheduler->L1->IsEmpty()){
        return;
    }
    ...
    //TODO
    this->run_time += kernel->stats->totalTicks - this->start_cpu_time;
    this->start_cpu_time = kernel->stats->totalTicks;
    //kernel->scheduler->preemtive = 0;
    nextThread = kernel->scheduler->FindNextToRun();
    if (nextThread != NULL) {
        DEBUG(dbgMp3, "[E] Tick ["<<kernel->stats->totalTicks<<"]: Thread  ["<< nextThread->getID() << "] is now selected for execution, thread [" << this->getID() << "] is replaced, and it has executed [" << this->run_time<< "] ticks");
        kernel->scheduler->ReadyToRun(this);
        kernel->scheduler->Run(nextThread, FALSE);
    }else{
        DEBUG(dbgSelf, "Tick [" << kernel->stats->totalTicks << "]: thread [" << this->getID() << "] is yield, and it has executed [" << this->run_time<< "] ticks");
    }
...
Thread::Sleep (bool finishing)
{
    ...
    //TODO
    int last_approximate_burst_time;
    last_approximate_burst_time = this->approximate_burst_time;
    this->run_time += kernel->stats->totalTicks - this->start_cpu_time;
    this->approximate_burst_time = (this->approximate_burst_time+this->run_time)/2.0;
    DEBUG(dbgMp3, "[D] Tick ["<<kernel->stats->totalTicks<<"]: Thread ["<<this->getID()<<"] update approximate burst time, from: ["<<last_approximate_burst_time<<"], add ["<<this->run_time<<"], to ["<<this->approximate_burst_time<<"]");
